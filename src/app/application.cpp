...
void resizeKernel(nlohmann::json& parameters, int size) {
    ...
    for (int y = -overlap; y <= overlap; ++y) for (int x = -overlap; x <= overlap; ++x) {
...
bool renderKernelEditor(nlohmann::json& parameters) {
    bool changed = false;
    int preset = 0;
    const auto presetName = parameters.value("preset", std::string("Custom"));
    for (int index = 1; index < static_cast<int>(kKernelPresetNames.size()); ++index)
        if (presetName == kKernelPresetNames[static_cast<std::size_t>(index)]) preset = index;
    ImGui::SetNextItemWidth(150);
    if (ImGui::Combo("Preset", &preset, kKernelPresetNames.data(), static_cast<int>(kKernelPresetNames.size()))) {
        applyKernelPreset(parameters, preset); changed = true;
    }
    int size = kernelSize(parameters);
    ImGui::SetNextItemWidth(150);
    if (ImGui::SliderInt("Kernel size", &size, 3, 15, "%d")) {
        size |= 1; resizeKernel(parameters, size); parameters["preset"] = "Custom"; changed = true;
    }
    ImGui::SameLine();
    ...
    for (int y = 0; y < size; ++y) {
        for (int x = 0; x < size; ++x) {
            ...
            if (ImGui::DragFloat("##weight", &values[index], 0.02F, -100.0F, 100.0F, "%.2g")) {
                writeKernel(parameters, values, size); parameters["preset"] = "Custom"; changed = true;
            }
            ImGui::PopID();
        }
    }
    bool normalize = parameters.value("normalize", 0.0F) > 0.5F;
    if (ImGui::Checkbox("Normalize weights", &normalize)) { parameters["normalize"] = normalize ? 1.0F : 0.0F; changed = true; }
    float bias = parameters.value("bias", 0.0F);
    ImGui::SetNextItemWidth(150);
    if (ImGui::DragFloat("Bias", &bias, 0.01F, -10.0F, 10.0F, "%.4g")) { parameters["bias"] = bias; changed = true; }
...
void Application::renderGraph() {
    ...
    for (auto& node : graph_.nodes()) {
        ...
        if (!descriptor) {
            ...
            if (node.type == "convolution" && renderKernelEditor(node.parameters)) dirty_ = true;
