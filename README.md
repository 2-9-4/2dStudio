...
The add-node menu contains **Reaction Diffusion (Monolithic)**, **Reaction Diffusion (Discrete)**, and **Generic Cellular Automata**. Adding an editable simulation creates a project-local subgraph; the cellular automaton uses a scalar State with Float-backed Birth/Survival integer masks (the default is Conway's `B3/S23`) and a Reset control. Select a node and press **Tab** to open its full-size graph, or press **Tab** again to return. Inside, use the normal editor interactions: right-click to add Float, Math, Bit Test, Threshold, Select, Canvas Coordinates, Laplacian, or Simulation Channel Split nodes, drag between named pins to add or replace links, select nodes or links and press Delete, and pan or zoom the canvas normally.
